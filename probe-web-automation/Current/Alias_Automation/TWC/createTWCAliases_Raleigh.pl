...
# Other Default Vars
my $probe = 'Raleigh';                # Name of the ouput file
...
$line = 1;
$sheet = 2; # Sheet 3 is the HOTG tab
...
$line = 1;
$sheet = 8; # Raleigh Tab; Fayetteville only uses some of the flows in this tab
			# Uses flows in this ad zone: Fayetteville, lumberton, southern pines
...
while( $ref->[$sheet]{'A'.$line} ne undef || $line < 10 ) {
	...
	if( $flowName ne "" &&
		...
		# Flowname with sheet prefix
		$flowName = 'Raleigh-'.$flowName;

		# Further filtering of what is in this sheet that gets addd in
		if( $ad_zone ne "Fayetteville" &&
		  $ad_zone ne "Lumberton" &&
		  $ad_zone ne "Southern Pines" ) {
...
while( $ref->[$sheet]{'A'.$line} ne undef || $line < 10 ) {
	...
	if( $primaryFlowName ne "" &&
		...
		# Flowname with sheet prefix
		$primaryFlowName = 'HOTG-'.$primaryFlowName;
...
while( $ref->[$sheet]{'A'.$line} ne undef || $line < 10 ) {
	...
	if( $primaryFlowName ne "" &&
		...
		# Flowname with sheet prefix
		$primaryFlowName = 'Raleigh-'.$primaryFlowName;
