...
# Other Default Vars
my $probe = 'Fayetteville';                # Name of the ouput file
...
while( $ref->[$sheet]{'A'.$line} ne undef || $line < 10 ) {
	...
	if( $flowName ne "" &&
		...
		# Flowname with sheet prefix
		$flowName = 'FAY-'.$flowName;

		if( $ad_zone eq "Fayetteville" ||
		  $ad_zone eq "Lumberton" ||
		  $ad_zone eq "Southern Pines" ) {
...
while( $ref->[$sheet]{'A'.$line} ne undef || $line < 10 ) {
	...
	if( $primaryFlowName ne "" &&
		...
		# Flowname with sheet prefix
		$primaryFlowName = 'FAY-'.$primaryFlowName;
